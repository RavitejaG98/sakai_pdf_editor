...
const App = () => {
  ...
  const loadPdfFromUrl = async (bloburl) => {
    try {
      console.log("modifiedPDFBlobURI", modifiedPDFBlobURI)
      // const loadingTask = pdfjs.getDocument(pdfURIModified);
      // const pdf = await loadingTask.promise;
      // console.log('pdf', pdf)
      let pdf
      if (modifiedPDFBlobURI == null) {
        const arrayBuffer = bloburl.buffer;
        console.log('null but', modifiedPDFBlobURI)
        pdf = await getDocument({ data: arrayBuffer }).promise;
      } else {
        console.log('not null but', modifiedPDFBlobURI)
        const loadingTask = pdfjs.getDocument(pdfURIModified);
  ...
  const drawTableOnPDF = async (pdfBytes, num, x, y) => {
    ...
    if (x != null && y != null) {
      ...
      pages.forEach((page) => {
        ...
        const numRows = 4;
        const numCols = 2;
        const rowHeight = (tableHeight / numRows) / zoomLevel;
        const colWidth = (tableWidth / numCols) / zoomLevel;
        /////////////////// row lines //////////////////
        for (let i = 0; i < 2; i++) {
          page.drawLine({
    ...
    console.log("pdfDoc", pdfDoc, "pdfDataUri", pdfDataUri)
    ...
    <div style={containerStyle}>
      <div style={gridContainerStyle}>
        <div style={gridItemStyle}>
          ...
          placeholder="Enter URL of the PDF file"
        ...
        /> */}
          ...
          <div>
            ...
            <div >
              ...
              <hr />
              {/* <input type="number" id="panXPercent" name="panXPercent" value={(panOffsetX / (canvasRef.current?.width / zoomLevel)) * 100} onChange={handlePanInputChange} /> */}
            ...
            {/* <Button  onClick={downloadPDF} className='bg-contain bg-cyan-400 p-2'>Download PDF with Table</Button > */}
          </div>
          <div className="mt-4">
